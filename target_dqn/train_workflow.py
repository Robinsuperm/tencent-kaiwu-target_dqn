...
# 生成随机宝箱和起点位置
def get_start_treasure(treasure_num: int, random_start: bool):
    treasures, start = [], 2
    if treasure_num > 0:
        treasures = random.sample(range(3, 16), treasure_num)
        treasures = sorted(treasures)
    if random_start:
        start_points = [2] + [i for i in range(3, 16) if i not in treasures]
        start = random.choice(start_points)
    return treasures, start
...
def workflow(envs, agents, logger=None, monitor=None):
    ...
    for epoch in range(epoch_num):
        ...
        ### curriculum learning
        if epoch < 300:
            usr_conf["diy"]["treasure_id"], usr_conf["diy"]["start"] = get_start_treasure(random.randint(0, 1), True)
        else :
            usr_conf["diy"]["treasure_id"], usr_conf["diy"]["start"] = get_start_treasure(9, True)
        ################################################################
...
def run_episodes(n_episode, env, agent, g_data_truncat, usr_conf, logger):
    for episode in range(n_episode):
        ...
        step = 0
        bump_cnt = 0
        _treasures_num = 0 ### 记录上一秒的treasures_num
        ...
        while not done:
            ...
            if env_info is None:
                ...
                reward, is_bump, reward_list = reward_shaping(
                    frame_no,
                    score,
                    terminated,
                    truncated,
        ...
        ################################################
        f = open('log/selflog.txt', 'a')
        f.write(f"撞墙次数:{bump_cnt}")
        f.write(f"宝箱数量：{treasures_num}\t")
        f.close()
